...
export const SalaryAdjustmentModal: React.FC<SalaryAdjustmentModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!reason.trim()) {
      toast('Vui lòng nhập lý do điều chỉnh lương thưởng!', 'error');
      return;
    }
    ...
    toast(`Đã lưu khoản điều chỉnh lương cho ${staff.name} thành công!`);
    onClose();
  };
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
            ...
            <div className="pt-2 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 border-t border-slate-100">
              <button
                type="button"
                onClick={onClose}
                className="w-full sm:w-auto px-4 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 text-center"
              >
                Hủy bỏ
              </button>
              <button
                type="submit"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold shadow-xs"
              >
